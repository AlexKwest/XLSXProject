...
namespace XLSXWPFForm
{
    public class LogicOperator
    {
        ...
        List<OperatorFirstMonthModel> operatorModelFirstMonth;
        List<OperatorTwoMonthModel> operatorModelTwoMonth;

        Dictionary<string, OperatorTwoMonthModel> resultUnionMonth;

        IEnumerable<OperatorFirstMonthModel> resultFirstMonth;

        public EnumResult.InputOklad inputOklad;

        #region Первая часть
        public LogicOperator(string FilePathIn)
        {
            excelIn = new ExcelPackage(new FileInfo(FilePathIn));
            excelOut = new ExcelPackage();
            operatorModelFirstMonth = new List<OperatorFirstMonthModel>();
        }

        public List<OperatorFirstMonthModel> SetOperatorFirstMonthList(EnumResult.InputOklad inputOklad)
        {
            this.inputOklad = inputOklad;
            GetFirtsListSheet(1);
            GetFirtsListSheet(2);
            return operatorModelFirstMonth;
        }

        private void GetFirtsListSheet(int numberSheet)
        {
            ...
            for (var row = 3; row < 100; row++)
            {
                ...
                {
                    operatorModelFirstMonth.Add(GetOperatorFirstMonth(row));
                }
                else break;
            }
        }
        ...
        {
            return (from t in operatorModelFirstMonth
                    where t.Name == MainWindow.poteriashka
                    select t.Finished).Sum();
        }

        private IEnumerable<OperatorFirstMonthModel> QueryTable()
        {
            return from t in operatorModelFirstMonth
                   where t.Name != MainWindow.poteriashka && (t.Day > 0 || t.Finished > 0)
                   select t;
        }
        ...
        private void PrintFirstPathMonth(string sheetName)
        {
            ...
            resultFirstMonth = QueryTable();
            foreach (var cell in resultFirstMonth)
            {
                newSheet.SetValue(row, col++, cell.Name);
                newSheet.SetValue(row, col++, cell.Day);
                newSheet.SetValue(row, col++, cell.Finished);
        ...
        public LogicOperator(string[] FilePathIn)
        {
            excelIn= new ExcelPackage(new FileInfo(FilePathIn[0]));
            excelInFirst = new ExcelPackage(new FileInfo(FilePathIn[1]));
            excelOut = new ExcelPackage();

            operatorModelFirstMonth = new List<OperatorFirstMonthModel>();
            operatorModelTwoMonth = new List<OperatorTwoMonthModel>();
        }

        #region Заполнение в лист модели из листа первой части месяца
        public List<OperatorFirstMonthModel> GetFirstMonth()
        {
            worksheet = excelInFirst.Workbook.Worksheets[1];

            for (var row = 2; row < 100; row++)
            {
                if (GetValueOperator(row))
                {
                    operatorModelFirstMonth.Add(SetFirstMonthWorksheet(row));
                }
                else break;
            }
            return operatorModelFirstMonth;
        }
        private OperatorFirstMonthModel SetFirstMonthWorksheet(int row)
        {
            var result = new OperatorFirstMonthModel();
            result.Name = worksheet.Cells[row, 1].Value.ToString();
            result.Day = Convert.ToInt32(worksheet.Cells[row, 2].Value);
            result.Finished = Convert.ToInt32(worksheet.Cells[row, 3].Value);
            result.Oklad = Convert.ToInt32(worksheet.Cells[row, 4].Value);
            result.Bonus = Convert.ToInt32(worksheet.Cells[row, 5].Value);
            result.OkladInPay = Convert.ToInt32(worksheet.Cells[row, 6].Value);
            result.BonusDyas = Convert.ToInt32(worksheet.Cells[row, 7].Value);
            result.Summa = Convert.ToInt32(worksheet.Cells[row, 8].Value);
            return result;
        }
        #endregion
        public List<OperatorTwoMonthModel> GetTwoMonth()
        {
            GetTwoListSheet(1);
            GetTwoListSheet(2);
            return operatorModelTwoMonth;
        }
        private Dictionary<string, OperatorTwoMonthModel> GetUnionDictynary()
        {
            UnionDictynary.UnionResult(operatorModelFirstMonth, operatorModelTwoMonth);
            return UnionDictynary.Result();
        }
        private void GetTwoListSheet(int numberSheet)
        {
            ...
            for (var row = 3; row < 100; row++)
            {
                if (GetValueOperator(row))
                {
                    operatorModelTwoMonth.Add(GetOperatorTwoMonth(row));
                }
                else break;
            }
        }
        private OperatorTwoMonthModel GetOperatorTwoMonth(int row)
        {
            var result = new OperatorTwoMonthModel();
            result.Name = worksheet.Cells[row, 1].Value.ToString();
            result.Day = Convert.ToInt32(worksheet.Cells[row, 130].Value);
            result.Finished = Convert.ToInt32(worksheet.Cells[row, 132].Value);
            return result;
        }
        private void PrintTwoPathMonth(string sheetName)
        {
            ...
            resultUnionMonth = GetUnionDictynary();
            //  resultFirstMonth = QueryTable();

            foreach (var cell in resultUnionMonth.Values)
            {
                newSheet.SetValue(row, col++, cell.Name);
                newSheet.SetValue(row, col++, cell.Day);
                newSheet.SetValue(row, col++, cell.Finished);
                newSheet.SetValue(row, col++, cell.FinishedAll);
                newSheet.SetValue(row, col++, cell.Oklad);
                ...
                newSheet.SetValue(row, col++, cell.Bonus15);
        ...
        private void StyleTable(string startHead, int col)
        {
            ...
            //TODO: Что-то придумать т.к. не работает для Второй части сохраниния.
            //   var modelRows = resultFirstMonth.Count() + 1;
            string modelRange = startHead + "61";//modelRows.ToString();
